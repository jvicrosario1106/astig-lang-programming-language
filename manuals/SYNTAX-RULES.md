...
| `const` read-only after init | Reassign `const` or `scan` into `const` |
| `//` and `/* */` comments | — |
...
**Don't — reassign `const` (type error):**
```astig
...
| Allowed | Not allowed |
|---------|-------------|
| Print, assign, scan, control flow, return | Standalone function call |
| `+=` / `-=` | `%=` and other compound ops |
| `print` as statement | `print` as expression |

**Do — call function inside print, scan, or assignment:**
```astig
pHR!HNTs(aHDs(1, 2));
...
| `+` `-` `*` `/` `%` | ASCII `&&` `\|\|` `!` |
| `==` `!=` `<` `>` `<=` `>=` | — |
| Jejemon logical: `nH0ts`, `aHNdz`, `0hrS` | — |
| String concat | Array-of-record types |
| Primitive `iHNtSZ[]` literals and indexing | Non-int array element types |
...
**Don't — ASCII logical operators (lex/parse error):**
```astig
!HFs(xH1s > 0 && yH2s > 0) {
  pHR!HNTs("both");
}
...
**Do — jejemon logical operators instead:**
```astig
!HFs((xH1s > 0) aHNdz (yH2s > 0)) {
  pHR!HNTs("both");
}
!HFs((xH1s > 10) 0hrS nH0ts (yH2s < 3)) {
  pHR!HNTs("or-not");
}
```
...
| Allowed | Not allowed |
|---------|-------------|
| `pHR!HNTs(...)` | `read` / other input keywords |
| `scH4nz("prompt", var)` or `scH4nz(var)` | `scan` into `const` |
...
| `&&` `\|\|` `!` | `aHNdz`, `0hrS`, `nH0ts` |
| `read` | `scH4nz` |
| plain English names | jejemon identifiers (`nH4mH3s`, `xH1s`) |
...
**With include + export (see `demo-examples/` or `test-case/`):**
```astig
iHNcHLuHD3s libHs.stg
